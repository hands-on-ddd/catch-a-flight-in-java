...
interface PasswordPolicy extends Function<Password, DomainPolicyOutput> {
    PasswordPolicy shouldContainsAtLeast12Characters = password -> {
    ...
    PasswordPolicy shouldContainsLowerCaseLetters = password -> {
    ...
    PasswordPolicy shouldContainsUpperCaseLetters = password -> {
        if (password.value().matches("[A-Z]+")) {
    ...
    PasswordPolicy shouldContainsDigits = password -> {
